...
class AsioClient:
    ...
    async def request(
        ...
        for attempt in range(_MAX_RETRIES + 1):
            try:
                ...
                if attempt < _MAX_RETRIES:
                    ...
                    continue
                raise AsioError(0, f"{e or type(e).__name__} (url={url})") from e
        ...
        if last_exc:
            raise AsioError(0, f"{last_exc}") from last_exc
        raise AsioError(0, "request failed with no response")
    ...
    def _raise_for_status(self, resp: httpx.Response) -> None:
        if resp.status_code >= 400:
            try:
                ...
                msg = resp.text
            raise AsioError(resp.status_code, str(msg)[:500])
